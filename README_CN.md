...
> **注意**：Aura 底层调用 Claude Code Agent SDK。请确保 `claude` 命令在 `PATH` 中可用，并且已完成认证（`claude login`）。
...
Aura 尚未进行代码签名，因此操作系统在首次打开时会显示安全警告。
...
1. 在访达中右键（或 Control-点击）`Aura.app`
2. 从右键菜单中选择 **打开**
3. 在确认对话框中点击 **打开**
...
- 聊天数据存储在 `~/.aura/aura.db`（开发模式下为 `./data/`）
