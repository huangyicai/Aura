...
const dataDir = process.env.CLAUDE_GUI_DATA_DIR || path.join(require('os').homedir(), '.aura');
const DB_PATH = path.join(dataDir, 'aura.db');
  ...
  if (!db) {
    ...
    if (!fs.existsSync(DB_PATH)) {
      ...
      const oldPaths = [
        ...
        path.join(home, 'Library', 'Application Support', 'Aura', 'aura.db'),
        path.join(home, 'Library', 'Application Support', 'aura', 'aura.db'),
        path.join(home, 'Library', 'Application Support', 'Claude GUI', 'aura.db'),
        // Old dev-mode fallback
        path.join(process.cwd(), 'data', 'aura.db'),
        // Legacy name
        path.join(home, 'Library', 'Application Support', 'Aura', 'claude-gui.db'),
        path.join(home, 'Library', 'Application Support', 'aura', 'claude-gui.db'),
      ];
