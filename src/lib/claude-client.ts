 ...
 * and return the file paths. The files are placed in .aura-uploads/
...
function saveUploadedFiles(files: FileAttachment[], workDir: string): string[] {
  const uploadDir = path.join(workDir, '.aura-uploads');
  ...
  return new ReadableStream<string>({
    async start(controller) {
      try {
        ...
        // Then overlay any API config the user set in Aura settings (optional).
